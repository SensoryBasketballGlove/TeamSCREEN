...
#define NUMBER_OF_SENSORS 10 //12
#define RED 22     
#define BLUE 24     
#define GREEN 23
#define LED_PWR 25
...
//      if ((buzzerCount != -1) && (buzzerCount < 50)) {
//        digitalWrite(2, HIGH);
...
void updateSensorValues() {
  ...
  if (IMU.accelerationAvailable()) {
    ...
    float pointFlexSensorValue = map(float(analogRead(A4)), 0.0, 1023.0, 0.0, 100.0);
    float middleFlexSensorValue = map(float(analogRead(A3)), 0.0, 1023.0, 0.0, 100.0);
    float ringFlexSensorValue = map(float(analogRead(A2)), 0.0, 1023.0, 0.0, 100.0);
    float pinkieFlexSensorValue = map(float(analogRead(A1)), 0.0, 1023.0, 0.0, 100.0);
